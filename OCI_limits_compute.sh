...
list_limits()
{
  ...
  do
    ...
    oci --profile $PROFILE limits value list --service-name compute --availability-domain $ad --compartment-id $TENANCYOCID --all --output table --query "data [*].{Shape:name, Number:value}" | while read myline
    do
