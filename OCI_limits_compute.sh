...
list_limits()
{
  ...
  do
    ...
    oci --profile $PROFILE limits value list --service-name compute --availability-domain eOnx:EU-FRANKFURT-1-AD-1 --compartment-id $TENANCYOCID --all --output table --query "data [*].{Shape:name, Number:value}" | while read myline
    do
