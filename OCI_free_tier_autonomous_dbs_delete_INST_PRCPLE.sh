...
Notes: 
- If --confirm is provided, found autonomous databases are deleted, otherwise only listed.
...
look_for_free_adbs()
{
  ...
  oci db autonomous-database list -c $lcompid --region $HOME_REGION --all \
       --query "data [?\"is-free-tier\" && \"lifecycle-state\" != 'TERMINATED'].{ADBid:id}" 2>/dev/null | jq -r '.[].ADBid' | \
  while read id
  do  
    if [ $CONFIRM == true ]; then 
      echo "`date '+%Y/%m/%d %H:%M'`: TERMINATING free autonomous database instance $id in compartment $lcompid"
      oci db autonomous-database delete  --autonomous-database-id $id --force > /dev/null 2>&1
    else
      echo "`date '+%Y/%m/%d %H:%M'`: FOUND free autonomous database instance $id in compartment $lcompid but not terminated since --confirm not provided"
    fi
  done
}
...
look_for_free_adbs $TENANCY_OCID 
