...
# - take a new snapshot for a compute instance (clone the boot volume and the block volumes and tag the clones volumes)
...
def get_alternate_source_volume_id(blkvol_attachments, source_blkvol_id, clone_blkvol_id):
    ...
    print (f"ERROR 12: cannot find the alternate source volume from clone volume {clone_blkvol_id} !")
    exit(12)
...
# - create a clone of the boot volume
# - create a clone of each attached block volume
...
def create_snapshot(instance_id, snapshot_name):
    ...
    # -- wait for the clone process to be completed (cannot add tags or delete VG before completion)
    ...
    # -- rename clone volumes and add tags to them
    tag_value = datetime.utcnow().strftime("%Y/%m/%d_%T")
    for id in clone_volume_ids:
...
def rollback_snapshot(instance_id, snapshot_name):
    ...
    # -- create new instance using clone boot volume
    ...
    # -- attach and rename clone block volume(s)
    if len(new_blkvol_ids) > 0:
        for blkvol_id in new_blkvol_ids:
            ...
            print (f"Attaching clone block volume ...{blkvol_id[-6:]} to new instance ...{new_instance_id[-6:]}")
            attach_block_volume_to_instance(blkvol_id, blkvol_attachments, new_instance_id, source_blkvol_id)

            print (f"Renaming clone block volume ...{blkvol_id[-6:]}")
