...
# --------------------------------------------------------------------------------------------------------------
# script OCI_limits_gpu.sh
#
# This script lists the compute limits for GPU in a specific region or all subscribed regions on an OCI tenant
...
#
# Versions
#    2021-09-29: Initial Version from OCI_limits_compute.sh
# --------------------------------------------------------------------------------------------------------------
...
list_limits()
{
  ...
  for ad in $ADS
  do
    printf "${COLOR_AD}   AD = %-40s${COLOR_NORMAL}\n" $ad > ${TMP_FILE}_AD_$ad
    ...
    done | egrep "Number|gpu2-count|gpu3-count|gpu4-count|\+-----">> ${TMP_FILE}_AD_$ad
